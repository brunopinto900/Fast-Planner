...
void rrtReplanFSM::init(ros::NodeHandle& nh) {
  current_wp_  = 0;
  ...
  /* callback */
  exec_timer_   = nh.createTimer(ros::Duration(0.01), &rrtReplanFSM::execFSMCallback, this);
  safety_timer_ = nh.createTimer(ros::Duration(0.05), &rrtReplanFSM::checkCollisionCallback, this);

  waypoint_sub_ =
      nh.subscribe("/waypoint_generator/waypoints", 1, &rrtReplanFSM::waypointCallback, this);
  odom_sub_ = nh.subscribe("/odom_world", 1, &rrtReplanFSM::odometryCallback, this);
...
void rrtReplanFSM::waypointCallback(const nav_msgs::PathConstPtr& msg) {
...
void rrtReplanFSM::odometryCallback(const nav_msgs::OdometryConstPtr& msg) {
...
void rrtReplanFSM::changeFSMExecState(FSM_EXEC_STATE new_state, string pos_call) {
...
void rrtReplanFSM::printFSMExecState() {
  string state_str[5] = { "INIT", "WAIT_TARGET", "GEN_NEW_TRAJ", "REPLAN_TRAJ", "EXEC_TRAJ" };

  cout << "[FSM]: state: " + state_str[int(exec_state_)] << endl;
}

void rrtReplanFSM::execFSMCallback(const ros::TimerEvent& e) {
...
void rrtReplanFSM::checkCollisionCallback(const ros::TimerEvent& e) {
  LocalTrajData* info = &planner_manager_->local_data_;
...
bool rrtReplanFSM::callRRTReplan() {
  bool plan_success =
      planner_manager_->rrtReplan(start_pt_, start_vel_, start_acc_, end_pt_, end_vel_);
